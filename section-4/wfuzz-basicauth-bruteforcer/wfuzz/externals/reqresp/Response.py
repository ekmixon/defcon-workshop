...
class Response:
	...
	def getCookie(self):
	    str = [j.split(";")[0] for i, j in self._headers if i.lower()=="set-cookie"]
	    return  "; ".join(str)


	def has_header(self,key):
	    return any(i.lower()==key.lower() for i, j in self._headers)
	
	def getLocation(self):
	    return next((j for i, j in self._headers if i.lower()=="location"), None)

	def header_equal(self,header,value):
	    return any(i==header and j.lower()==value.lower() for i, j in self._headers)
	...
	def getTextHeaders(self):
	    string = f"{str(self.protocol)} {str(self.code)} {str(self.message)}" + "\r\n"
	    for i,j in self._headers:
	        string += f"{i}: {j}" + "\r\n"
	...
	def parseResponse(self,rawResponse,type="curl"):
	    ...
	    if self.header_equal("Transfer-Encoding","chunked"):
	        ...
	        while nchunk := int(hexa.strip(), 16):
